...
class TestSuffixExtraction:
    """Tests for image filename suffix and extension parsing."""
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("B089WJC6Z4.MAIN.jpg", "MAIN", "jpg"),            # Strips existing 10-char ASIN prefix with dot
            ("B089WJC6Z4_PT02.png", "PT02", "png"),            # Strips existing 10-char ASIN prefix with underscore
...
class TestAsinParsing:
    """Tests for parsing, cleaning, and deduplicating ASINs."""
...
class TestImageManifestGeneration:
    """Tests for target path and manifest mapping."""

    def test_generate_image_manifest_cartesian_product(self):
        """Verify manifest generates Cartesian product of templates × ASINs in a single holistic list assertion."""
...
class TestImageDuplicationExecution:
    """Tests for on-disk file duplication and linking."""
...
class TestImageRenamerCli:
    ...
    def test_image_renamer_cli_execution(
        ...
        exec_result = subprocess.run(cmd, capture_output=True, text=True, check=True)

        assert exec_result.returncode == 0
        assert "AMAZON ASIN IMAGE DUPLICATOR" in exec_result.stdout
        assert (output_dir / "B011111111" / "B011111111.MAIN.jpg").exists()
        assert (output_dir / "B022222222" / "B022222222.MAIN.jpg").exists()
